        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form method="POST" action="{{ route('payments.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount') }}" required class="w-full border-gray-300 rounded-lg">
                    @error('amount')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="transaction_number" id="transaction_number" value="{{ old('transaction_number') }}" class="w-full border-gray-300 rounded-lg">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="observations" id="observations" class="w-full border-gray-300 rounded-lg">{{ old('observations') }}</textarea>
                    @error('observations')
                        <span class="text-red-600 text-xs">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">Registrar</button>
                </div>
            </form>
        </div>
    </div>
